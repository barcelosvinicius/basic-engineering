...
test('REGRESSION: a regex alternation is a pattern, never a pipe', () => {
  ...
  assert.strictEqual(ok, true, 'an alternation must match content, not be parsed as a shell pipe');
});
...
test('REGRESSION: the audit runs no shell at all', () => {
  ...
  assert.ok(
    !/\b(execSync|spawnSync|execFileSync)\s*\(/.test(src),
...
test('KNOWN POSITIVE: everyFile fails when a single file lacks the pattern', () => {
  ...
  assert.strictEqual(
    probes.run(dir, { kind: 'everyFile', dir: '.', name: /\.md$/, re: /^model:/m }),
    false,
    'one undeclared file must sink the check — that is what "every" means'
  );

  fs.writeFileSync(path.join(dir, 'c.md'), 'model: haiku\n');
  assert.strictEqual(probes.run(dir, { kind: 'everyFile', dir: '.', name: /\.md$/, re: /^model:/m }), true);
});
...
test('a failed check still says what it looked for', () => {
  ...
  assert.match(probes.describe({ kind: 'content', path: 'x.md', re: /pin|version/i }), /pin\|version/);
  assert.match(probes.describe({ kind: 'everyFile', dir: 'agents', name: /\.md$/, re: /^model:/m }), /agents/);
});
...
test('feedback/BACKLOG.md still matches what the probes measure', () => {
  ...
  const r = spawnSync(process.execPath, [path.join(ROOT, 'scripts', 'backlog-audit.js'), '--check'], {
    encoding: 'utf8',
  });
