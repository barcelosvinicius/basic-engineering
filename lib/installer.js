...
const UNIVERSAL_FILES = [
  ...
  { src: 'plugins/be/scripts', dest: 'scripts' },
...
/**
 * Seeded at the target project root when absent, never modified when present.
 * Without the pin, a Windows checkout (core.autocrlf=true) holds CRLF while
 * git, CI, and every other machine hold LF — and shell scripts, hashes, and
 * byte-level checks silently diverge per machine.
 */
const GITATTRIBUTES_PIN = /^\s*\*\s+text=auto\s+eol=lf/m;
const GITATTRIBUTES_CONTENT = [
  '# Seeded by basic-engineering: normalise line endings so the same commit',
  '# produces the same bytes on every machine. Without this, a Windows checkout',
  '# (core.autocrlf=true) holds CRLF while git and CI store LF — and shell',
  '# scripts, hashes, and byte-level checks silently diverge per machine.',
  '* text=auto eol=lf',
  '',
].join('\n');
 ...
 * @param {string}  [options.profile]       - Named install profile (which skills to copy);
 *   omitted means the manifest default. Undeclared here until 2026-09-23, although
 *   bin/be.js passed it and this function read it — the type checker named it on
 *   its first run.
...
function install(targetDir, options = {}) {
  ...
  const filesToCopy = action === 'fresh-install' ? [...UNIVERSAL_FILES, ...FRESH_ONLY_FILES] : UNIVERSAL_FILES;
  ...
  // ── Line-endings pin at the project root (fresh install and update) ───────
  ensureGitattributes(targetDir, log, warn);
...
/**
 * Seed .gitattributes at the project root so LF ships with the base.
 * Created only when absent; an existing file is NEVER modified — when it
 * lacks the pin, print an advisory and leave it alone.
 */
function ensureGitattributes(targetDir, log, warn) {
  const file = path.join(targetDir, '.gitattributes');
  if (!fs.existsSync(file)) {
    fs.writeFileSync(file, GITATTRIBUTES_CONTENT);
    log('  ✔  .gitattributes (project root — pins `* text=auto eol=lf`)');
    return;
  }
  if (!GITATTRIBUTES_PIN.test(fs.readFileSync(file, 'utf8'))) {
    warn('  ⚠️  .gitattributes exists but does not pin `* text=auto eol=lf`.');
    warn('      Consider adding that line so every machine checks out identical');
    warn('      bytes (your file was left untouched).');
  }
}
 ...
 * @param {(...args: any[]) => void} [warn] - Where an unknown profile is reported.
...
function resolveSkillFilter(profileName, warn = () => {}) {
  ...
  if (!profile) {
    warn(
      `  ⚠️  Unknown profile "${profileName}" — using the full set. Known: ${Object.keys(manifest.profiles).join(', ')}`
    );
