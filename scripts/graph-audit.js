...
const report = {
  ...
  hub: {
    name: hub,
    inDegree: inDeg[hub],
    outDegree: (node[hub] || { skills: [] }).skills.length,
    outEdges: (node[hub] || { skills: [] }).skills,
  },
  topInbound: Object.entries(inDeg)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 5),
  declaredEdges: Object.fromEntries(
    [...dGraph].filter(([, v]) => v.length).map(([k, v]) => [k, v.map((e) => `${e.type}:${e.target}`)])
  ),
...
const withResources = skills.filter((n) => fs.readdirSync(P('skills', n)).some((f) => f !== 'SKILL.md'));
const docsDir = path.join(ROOT, 'docs');
const livingDocs = fs.existsSync(docsDir) ? fs.readdirSync(docsDir).filter((f) => f.endsWith('.md')).length : 0;
...
function payloadBytes(files) {
  return files.reduce((n, f) => n + Buffer.byteLength(fs.readFileSync(f, 'utf8').replace(/\r\n/g, '\n'), 'utf8'), 0);
}
...
const defendedAgents = agentText.filter((t) => /prompt.injection|prompt defense|untrusted/i.test(t)).length;
...
// `--md` prints the block for a human to paste, which is fine for an edit made
// by hand and wrong for the one moment the panel is guaranteed to go stale: a
// release. The first row names the version, the release bumps the version, and
// `node --test` then fails on a row nothing regenerated — measured 2026-09-20,
// and the v3.1.1 run had already hit it and been patched by hand without a
// record. A generator that cannot write is a generator someone has to remember
// for.
if (process.argv.includes('--write')) {
  const doc = path.join(ROOT, 'docs', 'structural-analysis.md');
  if (!fs.existsSync(doc)) {
    console.error('graph-audit: docs/structural-analysis.md absent — nothing to write.');
    process.exit(1);
  }
  const lines = fs.readFileSync(doc, 'utf8').split(/\r?\n/);
  const start = lines.findIndex((l) => l.trim().startsWith('<!-- generated by scripts/graph-audit.js'));
  if (start === -1) {
    console.error(
      'graph-audit: no generated-block marker in docs/structural-analysis.md — refusing to guess where it goes.'
    );
    process.exit(1);
  }
  let end = start + 1;
  while (end < lines.length && lines[end].trim().startsWith('|')) end++;
  const block = [`<!-- generated by scripts/graph-audit.js on ${report.measuredOn} -->`, ...renderRows()];
  lines.splice(start, end - start, ...block);
  // LF is pinned for this repository (.gitattributes); writing CRLF here would
  // make the next --check compare bytes that only differ by line ending.
  fs.writeFileSync(doc, lines.join('\n'));
  console.log(`graph-audit: fact panel written to docs/structural-analysis.md (${block.length - 1} rows).`);
  process.exit(0);
}
...
if (process.argv.includes('--json')) {
  ...
  console.log(
    `  hub ${hub}: in=${report.hub.inDegree} out=${report.hub.outDegree} -> ${report.hub.outEdges.join(', ') || '—'}`
  );
  console.log(`  top inbound: ${report.topInbound.map(([k, v]) => `${k}(${v})`).join(' · ')}`);
  console.log(
    `  hub declared edges           : ${dOut.length}  ${dOut.map((e) => `${e.type}:${e.target}`).join(', ') || '—'}`
  );
  console.log(
    `  invoke cycles                : ${report.invokeCycles.length === 0 ? 'none' : report.invokeCycles.map((c) => c.join('->')).join(' | ')}`
  );
}
