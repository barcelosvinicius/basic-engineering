...
| **Guardrail hooks** | live | **Fail-open, opt-out** (`BE_HOOKS=off`): PreToolUse blocks hardcoded secrets, weakening linter configs, and `git --no-verify`; a fact-forcing gate on the first edit of a high-impact file — schema, security, API contract, build manifest, pipeline (`BE_GATEGUARD=all|off`); a Stop reminder for `/be:session-end`; SessionStart injects `HISTORY` state and drops the capabilities guide at the project root |
