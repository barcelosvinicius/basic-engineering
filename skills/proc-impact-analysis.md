...
description: >
  Pre-PR change impact analysis — given a set of modified files, determines which
  modules, flows, domain events, and downstream consumers are affected, assigns a
  risk level, and produces a focused review checklist. Use before opening a PR,
  after a large refactoring, or whenever a change touches a shared boundary.
---
...
This is the pre-PR complement to `proc-code-review.md` — impact analysis scopes
...
- The project's domain map (from `proc-domain-mapping.md` output)
